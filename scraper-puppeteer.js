  ...
  filters: {
    ...
    url_replace: function (value) {
      return typeof value === "string"
        ? value.replace("/121x90/", "/900x550/")
        : value;
    },
    // Gallery thumbs url example: "https://www.cooperklima.hu/img/82802/CH-S09FTXLA2-NG_altpic_2/100x100/CH-S09FTXLA2-NG.jpg?time=1721301797"
    // replace the thumb URL with the full-size image URL
    gallery_thumbs_replace: function (value) {
      return typeof value === "string"
        ? value.replace("/100x100/", "/")
        : value;
    },
  },
})
...
// Variable to control the number of pages to scrape
const maxPagesToScrape = 1; // Set this to the number of pages you want to scrape
...
x(mainUrl, [".categories .category-card__pic-url@href"])((err, urls) => {
  ...
  if (urls && Array.isArray(urls)) {
    familyUrls.push(...urls.slice(0, maxPagesToScrape)); // Limit the number of pages to scrape
  ...
  familyUrls.forEach((familyUrl) => {
    x(familyUrl, [".page_artlist_list .product .product_link_normal@href"])(
      (err, urls) => {
        if (err) {
          console.error(
            `Error fetching product URLs from ${familyUrl}: ${err}`
          );
          return;
        }
        if (urls && Array.isArray(urls)) {
        ...
        (async () => {
          ...
          for (const productUrl of productUrls) {
            try {
              ...
              const imageSelector = ".js-init-ps";
              ...
              if (imageExists) {
                ...
                try {
                  ...
                  await page.evaluate(() => {
                    ...
                    console.log("PhotoSwipe is initialized");
                  });
                  ...
                  console.error(
                    `Error loading PhotoSwipe gallery on ${productUrl}: ${error}`
                  );
                }
              } else {
              ...
              x(productUrl, "#page_artdet_content", {
                ...
                galleryThumbs: x(".artdet__img--thumb", ["@src"]),
                gallery: x(".artdet__img--thumb", ["@src | gallery_thumbs_replace"]),
                ...
                performance: x(
                  "#artdet__type .product-type__item.type--text .product-type__values .product-type__value",
                  [
                    {
                ...
                color: x(
                  "#artdet__type .product-type__item.type--icon .product-type__values .product-type__value",
                  [
                    {
                ...
                if (err) {
                  console.error(
                    `Error fetching product details from ${productUrl}: ${err}`
                  );
                  return;
                }
                ...
                const mainAttributes = transformAttributes(
                  product.mainAttributes
                );
                const furtherAttributes = transformAttributes(
                  product.furtherAttributes
                );
                ...
                product.performance = product.performance.map((item) =>
                  item.name.trim()
                );
              ...
              console.error(
                `Error fetching product details from ${productUrl}: ${error}`
              );
            }
          }
